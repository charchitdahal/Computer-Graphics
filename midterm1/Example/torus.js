    ...
    if(type == "mesh"){
        ...
        for(var i = 0; i < nRows; i++)
        {
          for(var j = 0; j < nColumns; j++)
          {
            ...
            var xyz = get_xyz(r_data, R_data, theta_data, phi_data);
        ...
        for(var j = 0; j < nColumns; j++) for(var i = 0; i < nRows; i++)
        {
          ...
          theta_data = 2 * Math.PI * ((i/(nColumns - 1)) - (1/2));
          var xyz = get_xyz(r_data, R_data, theta_data, phi_data);
          pointsArray.push(xyz);
          var x = (1 + xyz[1]) / 2;
          colors.push(find_color(colormap, (1 - x, x)));
    ...
    // "surface"
        ...
        for(var i = 0; i < nRows - 1; i++)
        {
            for(var j = 0; j < nColumns - 1; j++)
            {
              ...
              var xyz1 = get_xyz(r_data, R_data, theta_data1, phi_data1);
              pointsArray.push( xyz1 );
              x = (1 + xyz1[1]) / 2;
              colors.push(find_color(colormap, (1 - x, x)));

              var xyz2 = get_xyz(r_data,R_data, theta_data2, phi_data1);
              pointsArray.push( xyz2 );
              x = (1 + xyz2[1]) / 2;
              colors.push(find_color(colormap, (1 - x, x)));

              var xyz3 = get_xyz(r_data,R_data, theta_data2, phi_data2);
              pointsArray.push( xyz3 );
              x = (1 + xyz3[1]) / 2;
              colors.push(find_color(colormap, (1 - x, x)));

              var xyz4 = get_xyz(r_data, R_data, theta_data1, phi_data2);
...
function find_color(colormap,ratio){
    switch (colormap){
        // from magenta, through yellow, back to magenta.
        case "spring":
            if(ratio<0.5) return mix(magenta,yellow,ratio*2);
            else return mix(yellow,magenta,(ratio-0.5)*2);
        case "summer":
            if(ratio<0.5) return mix(green,yellow,ratio*2);
            else return mix(yellow,green,(ratio-0.5)*2); 
        case "autumn":
            if(ratio<0.5) return mix(red,orange,ratio*2);
            else return mix(orange,red,(ratio-0.5)*2);
        case "winter":
            if(ratio<0.5) return mix(black,blue,ratio*2);
            else return mix(blue,black,(ratio-0.5)*2);       
        // add your code here for other three colors
        // your code goes here:
    }
}
...
function get_xyz(r, R, theta, phi){
    return vec4(
